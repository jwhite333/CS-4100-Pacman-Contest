...
from DQNTutorial import ReplayMemory, resize, Transition
...
class DQN(nn.Module):
  def __init__(self, h, w, outputs):
    ...
    self.head = nn.Linear(204288, outputs)
...
def runGames( layout, agents, display, length, numGames, record, numTraining, pacmanTeamName, ghostTeamName, muteAgents=False, catchExceptions=True ):
  ...
  for i in range( numGames ):
    ...
    for t in count():
      ...
      # Get action
      pacmanAction = None
      for index, (agentIndex, action) in enumerate(g.moveHistory[moveHistoryIter:]):
        if agentIndex == 0:
